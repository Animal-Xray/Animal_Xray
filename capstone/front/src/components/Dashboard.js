...
function Dashboard() {
  ...
  const fetchWorkspaces = async () => {
    try {
      const response = await axios.get("http://192.168.0.168:8080/api/workspace");
  ...
  const createWorkspace = async (e) => {
    ...
    try {
      await axios.post("http://192.168.0.168:8080/api/workspace/create", workspaceName,{
  ...
  const addFileToWorkspace = async (workspaceId) => {
    ...
    try {
      await axios.post("http://192.168.0.168:8080/api/photos/upload", formData, {
  ...
  const fetchPhotos = async (workspaceId) => {
    try {
      const response = await axios.get(
        `http://192.168.0.168:8080/api/photos/${workspaceId}`
      );
      setSelectedWorkspace(workspaceId); // 선택된 워크스페이스 설정
      setPhotos(response.data); // 사진 목록 업데이트

       // 선택된 워크스페이스와 사진 목록을 ImageList 컴포넌트로 전달
      navigate(`/${workspaceId}`, { state: { workspace: { id: workspaceId, photos: response.data } } });
  ...
  // 워크스페이스 삭제 요청
	const deleteWorkspace = async (workspaceId) => {
		...
		try {
			await axios.delete(`http://192.168.0.168:8080/api/workspace/${workspaceId}`);
